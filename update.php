...
$course = $_POST['course'];
$id=uniqid();
$q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$name' , '$sahi' , '$wrong','$total','$time' ,'$desc','$tag', NOW() ,'$email','$course')");
...
if($sn == 1)
{
$q=mysqli_query($con,"INSERT INTO history VALUES('$email','$eid' ,'0','0','0','0',NOW(),'$course')")or die('Error');
}
...
if($sn == 1)
{
$q=mysqli_query($con,"INSERT INTO history VALUES('$email','$eid' ,'0','0','0','0',NOW(),'$course' )")or die('Error137');
}
...
{
$sn++;
header("location:account.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total&course=$course")or die('Error152');
}
...
header("location:account.php?q=result&eid=$eid&course=$course");
}
else
{
header("location:account.php?q=result&eid=$eid&course=$course");
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:account.php?q=quiz&step=2&eid=$eid&n=1&t=$t&course=$course");
}
